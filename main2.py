...
"""
PDF to JSON Processor with FastAPI - Enhanced with Reference Document Support

This application provides an API endpoint for:
1. Uploading PDF documents
2. Extracting text from PDFs
3. Processing text using GPT-4o Mini with field description guidance
4. Populating a JSON template with extracted information
5. Returning the populated JSON

Implementation details:
- Intelligent text chunking with 10% overlap between chunks to preserve context continuity
- Optimized chunk sizing to maximize information density while respecting token limits
- Reference document integration (reference.xlsx) for enhanced field extraction accuracy
- Field description mapping to improve LLM prompt effectiveness
- Both synchronous and asynchronous processing options to handle documents of all sizes
- Smart conflict resolution when merging data from multiple chunks
- Comprehensive error handling at each processing stage
- Support for complex nested JSON structures with recursive field mapping
- Automatic temporary file management and cleanup
- Efficient token usage with tiktoken for accurate counting
"""
...
def load_field_descriptions(reference_path: str = "reference.xlsx") -> Dict[str, str]:
    """
    Load field descriptions from a reference Excel file if it exists.
    Expected format: Column A = field name, Column B = field description
    """
    try:
        if os.path.exists(reference_path):
            ...
            if len(df.columns) >= 2:
                ...
                for _, row in df.iterrows():
                    ...
                    field_desc = str(row.iloc[1]).strip(
                    ) if pd.notna(row.iloc[1]) else ""
                    if field_name and field_name.lower() != "nan":
                        field_descriptions[field_name] = field_desc

                logger.info(
                    f"Loaded {len(field_descriptions)} field descriptions from {reference_path}")
                return field_descriptions
            else:
                logger.warning(
                    f"Reference file {reference_path} doesn't have the expected format")
                return {}
        else:
            logger.info(f"Reference file {reference_path} not found")
            return {}
    except Exception as e:
        logger.error(f"Error loading field descriptions: {e}")
        return {}
...
def process_chunk_with_llm(
    ...
    If field_descriptions is provided, they will be included in the prompt.
    """
    # Construct the field guidance section if descriptions are available
    field_guidance = ""
    if field_descriptions and len(field_descriptions) > 0:
        field_guidance = "FIELD DESCRIPTIONS:\n"
        for field in template_fields:
            if field in field_descriptions and field_descriptions[field]:
                field_guidance += f"- {field}: {field_descriptions[field]}\n"
            else:
                field_guidance += f"- {field}: (No specific description available)\n"
...
async def process_pdf_background(
    ...
    model: str = "gpt-4o-mini",
    reference_path: str = "reference.xlsx"
):
    ...
    try:
        ...
        # Load field descriptions from reference file if it exists
        field_descriptions = load_field_descriptions(reference_path)
...
async def upload_pdf(
    ...
    use_reference: bool = Query(
        True, description="Whether to use reference.xlsx for field descriptions")
):
    """
    ...
    - If use_reference is True, field descriptions from reference.xlsx will be used in the prompt
    """
    ...
    reference_path = "reference.xlsx" if use_reference else None
...
async def process_pdf(
    ...
    try:
        ...
        # Load field descriptions if requested
        field_descriptions = {}
        if use_reference and os.path.exists("reference.xlsx"):
            field_descriptions = load_field_descriptions("reference.xlsx")
